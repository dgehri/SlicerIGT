...
void vtkSlicerVolumeResliceDriverLogic
::SetOffsetForSlice(double offset, vtkMRMLSliceNode* sliceNode)
{
    if (sliceNode == NULL)
    {
        return;
    ...
    offsetSs << offset;
    sliceNode->SetAttribute(VOLUMERESLICEDRIVER_OFFSET_ATTRIBUTE, offsetSs.str().c_str());
...
::UpdateSlice(vtkMatrix4x4* driverToRASMatrix, vtkMRMLSliceNode* sliceNode)
{
    ...
    int mode = MODE_NONE;
    if (auto modeCC = sliceNode->GetAttribute(VOLUMERESLICEDRIVER_MODE_ATTRIBUTE))
    {
    ...
    double rotation = 0.0;
    if (auto rotationCC = sliceNode->GetAttribute(VOLUMERESLICEDRIVER_ROTATION_ATTRIBUTE))
    {
    ...
    int flip = 0;
    if (auto flipCC = sliceNode->GetAttribute(VOLUMERESLICEDRIVER_FLIP_ATTRIBUTE))
    {
    ...
    double offset = 0.0;
    if (auto offsetCC = sliceNode->GetAttribute(VOLUMERESLICEDRIVER_OFFSET_ATTRIBUTE))
    {
        std::stringstream offsetSs(offsetCC);
        offsetSs >> offset;
    }


    // convert driver transform to vtkTransform
    auto driverToRASTransform = vtkSmartPointer< vtkTransform >::New();
    driverToRASTransform->SetMatrix(driverToRASMatrix);
    driverToRASTransform->Update();

    // extract translation as vtkTransform
    double driverToRasTranslationVector[3];
    driverToRASTransform->GetPosition(driverToRasTranslationVector);
    auto driverToRasTranslation = vtkSmartPointer< vtkTransform >::New();
    ...
    // extract rotation matrix as vtkTransform
    double driverToRasOrientationVector[4];
    driverToRASTransform->GetOrientationWXYZ(driverToRasOrientationVector);
    auto driverToRasRotation = vtkSmartPointer< vtkTransform >::New();
    driverToRasRotation->Identity();
    driverToRasRotation->RotateWXYZ(
        driverToRasOrientationVector[0],
        driverToRasOrientationVector[1],
        driverToRasOrientationVector[2],
        driverToRasOrientationVector[3]);
    driverToRasRotation->Update();

    // prepare result matrix
    auto sliceToRASTransform = vtkSmartPointer< vtkTransform >::New();
    ...
    case MODE_SAGITTAL:
    {
        ...
        sliceToRASTransform->RotateY(90.0); // first, rotate to sagittal plane.
        sliceToRASTransform->Update();
        break;
    }
    case MODE_CORONAL:
    {
        ...
        sliceToRASTransform->RotateX(90.0); // First, rotate to coronal plane.
        sliceToRASTransform->Update();
        break;
    }
    case MODE_INPLANE:
    {
        auto sliceToDriverTransform = vtkSmartPointer< vtkTransform >::New();
        sliceToDriverTransform->Identity();
        sliceToDriverTransform->RotateX(-90.0);
        sliceToDriverTransform->RotateY(90.0);
    ...
    case MODE_TRAJECTORY_1: // longitudinal view along path
    {
        // move slice to tool tip (origin)
        sliceToRASTransform->Concatenate(driverToRasTranslation);

        // calculate angle of tool z-axis in sagittal (y/z) plane and rotate about x-axis
        double tilt = vtkMath::DegreesFromRadians(atan2(driverToRASMatrix->GetElement(2, 2), driverToRASMatrix->GetElement(1, 2)));
        sliceToRASTransform->RotateX(tilt);

        // rotate about z-axis (coronal plane)
        sliceToRASTransform->RotateZ(rotation);

        // flip about X (sagittal plane)
        sliceToRASTransform->RotateX(flip * 180.0);

        sliceToRASTransform->Update();
        break;
    }
    case MODE_TRAJECTORY_2: // longitudinal view along path, perpendicular to MODE_TRAJECTORY_1
    {
        // move slice to tool tip (origin)
        sliceToRASTransform->Concatenate(driverToRasTranslation);

        // rotate 90 degrees about tool axis
        sliceToRASTransform->RotateWXYZ(90.0, driverToRASMatrix->GetElement(0, 2), driverToRASMatrix->GetElement(1, 2), driverToRASMatrix->GetElement(2, 2));
    ...
    case MODE_PROBE_VIEW:   // like MODE_TRANSVERSE, but taking into account a user configured offset
    {
        // translate along tool axis
        vtkVector3d toolAxis(driverToRASMatrix->GetElement(0, 2), driverToRASMatrix->GetElement(1, 2), driverToRASMatrix->GetElement(2, 2));
        toolAxis.Normalize();
        sliceToRASTransform->Translate(toolAxis.GetX() * offset, toolAxis.GetY() * offset, toolAxis.GetZ() * offset);

        // move slice to tool tip (origin)
        sliceToRASTransform->Concatenate(driverToRasTranslation);

        // rotate about x to make plane normal to tool trajectory
        sliceToRASTransform->RotateX(90.0);
